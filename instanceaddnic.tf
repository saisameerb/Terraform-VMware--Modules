...
resource "vsphere_virtual_machine" "addtlnic" {
  ...
  clone {
    ...
    customize {
      ...
      network_interface {
        ...
        ipv4_netmask = "${lookup(var.vms[count.index], "net2_prefix",  var.default["net2_prefix"])}"
      }
      #VM customization under the vsphere_virtual_machine resource does not take a per-interface gateway setting and only default routes are configured on a global basis
